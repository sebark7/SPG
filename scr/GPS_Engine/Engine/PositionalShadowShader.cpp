...
PositionalShadowShader::PositionalShadowShader()
{
	m_vertexShader = 0;
	m_pixelShader = 0;
	m_layout = 0;
...
PositionalShadowShader::PositionalShadowShader(const PositionalShadowShader& other)
{
}
...
bool PositionalShadowShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;


	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, L"../Engine/positionalshadow.vs", L"../Engine/positionalshadow.ps");
	if (!result)
	{
		return false;
	}
...
void PositionalShadowShader::Shutdown()
{
...
bool PositionalShadowShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
...
bool PositionalShadowShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "ShadowVertexShader", "vs_5_0",
...
void PositionalShadowShader::ShutdownShader()
{
...
void PositionalShadowShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool PositionalShadowShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
	const XMMATRIX& projectionMatrix, const XMMATRIX& lightViewMatrix, const XMMATRIX& lightProjectionMatrix,
	ID3D11ShaderResourceView* depthMapTexture, XMFLOAT3 lightPosition)
{
	HRESULT result;
...
void PositionalShadowShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
